...
func newRootCommand() *cobra.Command {
	root := &cobra.Command{
		Use:           "go-pie",
		Short:         "🐹🥧 go-pie — install PHP extensions (a Go port of PIE, Docker-aware)",
		Version:       version.Version,
		SilenceErrors: true,
		SilenceUsage:  true,
		RunE: func(cmd *cobra.Command, args []string) error {
			// Bare `go-pie` is a usage error (a subcommand is required). Leaving
	...
	pf.StringVar(&phpPath, "with-php-path", "", "Path to the target `php` binary (defaults to the first on $PATH)")
