...
func Resolve(ctx context.Context, client PackageSource, req *RequestedPackage, plat *platform.TargetPlatform) (*ResolvedPackage, error) {
	versions, err := client.PackageVersions(ctx, req.Name)
	if err != nil {
	...
	if err := ValidateExtensionName(extensionName); err != nil {
		return nil, fmt.Errorf("invalid php-ext metadata for `%s:%s`: %w",
			req.Name, chosen.Version, err)
	}
