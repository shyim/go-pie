...
// swallowed).
func TargetPlatformFromPhp(ctx context.Context, php *PhpBinary, makeJobs *int, phpize *PhpizePath) (*TargetPlatform, error) {
	os := currentOS()
	threadSafety := NonThreadSafe
	if php.ThreadSafe {
		threadSafety = ThreadSafe
	}
	phpConfig := guessPhpConfig(ctx, php)
	if phpize == nil {
		if guessed, err := GuessPhpize(ctx, php); err == nil {
			phpize = guessed
		}
	}
	jobs := runtime.NumCPU()
