...
func TestBuildRejectsPrePackagedBinary(t *testing.T) {
	...
	_, err := Build(t.Context(), pkg, src, plat, nil)
	if err == nil {
...
func TestBuildSucceedsEndToEnd(t *testing.T) {
	...
	built, err := Build(t.Context(), pkg, src, plat, nil)
	if err != nil {
...
func TestBuildWithSinkCapturesOutput(t *testing.T) {
	...
	_, err := BuildWith(t.Context(), pkg, src, plat, nil, 1, &sink)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	out := sink.String()
...
func TestBuildConfigurePassesPhpConfig(t *testing.T) {
	...
	_, err := Build(t.Context(), pkg, src, plat, []string{"--enable-redis-igbinary", "--with-foo"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
