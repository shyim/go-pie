...
func ExpectedFromGithubDigest(digest *string) Expected {
	if digest != nil {
		// Trim before and after the prefix: stray whitespace would otherwise
		// either skip verification entirely or fail every valid artifact.
		trimmed := strings.TrimSpace(*digest)
		if hexPart, ok := strings.CutPrefix(trimmed, "sha256:"); ok {
