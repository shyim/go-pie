...
var verifyGithubAttestationFile = VerifyGithubAttestation
...
// Windows download.
func (d *DownloadedPackage) WindowsDll() (string, string, bool) {
...
func Download(ctx context.Context, pkg *resolver.ResolvedPackage, plat *platform.TargetPlatform, policy VerifyPolicy) (*DownloadedPackage, error) {
	var errs []string
	...
	for _, method := range methods {
		downloaded, err := tryDownloadWith(ctx, pkg, plat, method, policy)
		if err == nil {
...
func tryDownloadWith(ctx context.Context, pkg *resolver.ResolvedPackage, plat *platform.TargetPlatform, method resolver.DownloadUrlMethod, policy VerifyPolicy) (*DownloadedPackage, error) {
	switch method {
	case resolver.ComposerDefault:
		...
		if err := applyVerification(ctx, b, expected, policy, url, method, ""); err != nil {
			return nil, err
		}
	...
	case resolver.PrePackagedSource:
		...
		asset, err := findReleaseAsset(ctx, pkg, names)
		if err != nil {
			return nil, err
		}
		distType := distTypeFromURL(asset.DownloadURL)
		b, err := fetch(ctx, asset.DownloadURL)
		if err != nil {
			return nil, err
		}
		expected := ExpectedFromGithubDigest(asset.Digest)
		if err := applyVerification(ctx, b, expected, policy, asset.DownloadURL, method, asset.Repo); err != nil {
			return nil, err
		}
	...
	case resolver.WindowsBinary:
		...
		if len(names) == 0 {
			return nil, errors.New("could not determine the Windows build tag (compiler) for the target PHP; is this a Windows PHP build?")
		}
		asset, err := findReleaseAsset(ctx, pkg, names)
		if err != nil {
			return nil, err
	...
	default:
		return nil, errors.New("unknown download method")
	}
}
...
func applyVerification(ctx context.Context, b []byte, expected Expected, policy VerifyPolicy, url string, method resolver.DownloadUrlMethod, attestRepo string) error {
	...
	if policy == VerifyAttest && attestRepo != "" {
		attestFile, err := os.CreateTemp("", "gpie-attest-*")
		if err != nil {
			return fmt.Errorf("staging artifact for GitHub attestation verification: %w", err)
		}
		attestPath := attestFile.Name()
		defer func() { _ = os.Remove(attestPath) }()
		if _, err := attestFile.Write(b); err != nil {
			_ = attestFile.Close()
			return fmt.Errorf("staging artifact for GitHub attestation verification: %w", err)
		}
		if err := attestFile.Close(); err != nil {
			return fmt.Errorf("staging artifact for GitHub attestation verification: %w", err)
		}

		ok, aerr := verifyGithubAttestationFile(ctx, attestPath, attestRepo)
		switch {
		case aerr != nil:
...
func fetch(ctx context.Context, url string) ([]byte, error) {
	...
	buf, err := readLimited(resp.Body, maxArtifactDownloadBytes)
	if err != nil {
