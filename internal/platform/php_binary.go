...
// PhpBinaryFromPathEnv locates `php` on $PATH and introspects it. When $PATH
// holds no `php`, it falls back to scanning the known PHP installation
// locations (Homebrew, Herd, asdf, Ondrej PPA, ...).
func PhpBinaryFromPathEnv(ctx context.Context) (*PhpBinary, error) {
	path, err := exec.LookPath("php")
	if err != nil {
		return PhpBinaryDiscovered(ctx)
	}
	return PhpBinaryFromPath(ctx, path)
}
...
func PhpBinaryFromPath(ctx context.Context, path string) (*PhpBinary, error) {
	var (
		wg                       sync.WaitGroup
	...
	go func() {
		defer wg.Done()
		phpinfo, phpinfoErr = runPhp(ctx, path, []string{"-i"})
	}()
	go func() {
		defer wg.Done()
		constants, constantsErr = runPhp(ctx, path, []string{"-n", "-r", constScript})
	}()
	wg.Wait()
	...
	if !ok {
		return nil, errors.New("could not determine PHP API version from `php -i`")
	}
...
func (p *PhpBinary) LoadedExtensions(ctx context.Context) ([]string, error) {
	raw, err := runPhp(ctx, p.Path, []string{
...
func (p *PhpBinary) LoadedExtensionsWithVersions(ctx context.Context) ([]ExtensionVersion, error) {
	raw, err := runPhp(ctx, p.Path, []string{
		"-d", "display_errors=stderr",
...
func (p *PhpBinary) ExtensionIsLoaded(ctx context.Context, name string) (bool, error) {
	needle := strings.ToLower(name)
	loaded, err := p.LoadedExtensions(ctx)
	if err != nil {
		return false, err
	}
...
func runPhp(ctx context.Context, path string, args []string) (string, error) {
...
func parseConstants(raw string) (phpConstants, bool) {
	...
	if strings.TrimSpace(lines[4]) != "GPIE_OK" {
		return phpConstants{}, false
	}
	return phpConstants{
		version:    version,
