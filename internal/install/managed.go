...
// RpieMarkerPrefix is the marker comment RPIE writes above every directive it
// manages. `discover` scans for this exact prefix; keep writer and scanner in
// sync — this is the single source of truth for "did rpie install this?".
const RpieMarkerPrefix = "; rpie automatically added this to enable the "

// ManagedExtension is a single extension RPIE set up, as reconstructed from its
// INI snippet.
...
// DiscoverManaged finds every RPIE-managed extension for the target platform,
...
func DiscoverManaged(plat *platform.TargetPlatform) (map[string]ManagedExtension, error) {
	...
	files, err := candidateIniFiles(plat)
	if err != nil {
		return nil, err
	}
...
func candidateIniFiles(plat *platform.TargetPlatform) ([]string, error) {
	var files []string
...
// parseMarkers parses all RPIE markers in one INI file's contents.
func parseMarkers(contents, iniFile string) []ManagedExtension {
	...
	for idx, line := range lines {
		...
		rest, ok := strings.CutPrefix(trimmed, RpieMarkerPrefix)
		if !ok {
			continue
		}
...
// RemoveMarkerBlock removes the RPIE-managed block for extensionName from
...
func RemoveMarkerBlock(contents, extensionName string) string {
	...
	for i < len(lines) {
		...
		if rest, ok := strings.CutPrefix(trimmed, RpieMarkerPrefix); ok {
