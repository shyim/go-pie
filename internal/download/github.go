...
func findReleaseAsset(ctx context.Context, pkg *resolver.ResolvedPackage, candidateNames []string) (*releaseAsset, error) {
	repo, err := githubOrgAndRepo(pkg)
	if err != nil {
		return nil, err
	}
	// Tags legitimately contain characters that are reserved in a URL path
	// (notably '+' in semver build metadata), so escape rather than interpolate.
	url := fmt.Sprintf("%s/repos/%s/releases/tags/%s", githubAPI, repo, neturl.PathEscape(pkg.Version))

	response, err := getJSON(ctx, url)
	if err != nil {
...
func getJSON(ctx context.Context, url string) (map[string]any, error) {
	...
	if resp.StatusCode == http.StatusNotFound {
		return nil, errors.New("no GitHub release found for that version tag (HTTP 404)")
	}
