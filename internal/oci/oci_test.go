...
func TestReadsCellFromDescriptorAnnotation(t *testing.T) {
	desc := mustJSONMap(t, `{
		"digest": "sha256:aaa",
		"annotations": {"sh.gpie.cell": "redis/6.3.0/php8.4/debian@12/x86_64/nts/nodebug/cfg-00000000"}
	}`)
	cell, ok := descriptorCell(desc)
	if !ok || cell != "redis/6.3.0/php8.4/debian@12/x86_64/nts/nodebug/cfg-00000000" {
		t.Errorf("descriptorCell = %q, %v", cell, ok)
	}
...
func TestReadsCellFromManifestAnnotation(t *testing.T) {
	manifest := mustJSONMap(t, `{
		"config": {"digest": "sha256:c"},
		"layers": [{"digest": "sha256:l"}],
		"annotations": {"sh.gpie.cell": "amqp/v2.2.0/php8.4/debian@13/aarch64/nts/nodebug/cfg-00000000"}
	}`)
	cell, ok := manifestCell(manifest)
	if !ok || cell != "amqp/v2.2.0/php8.4/debian@13/aarch64/nts/nodebug/cfg-00000000" {
		t.Errorf("manifestCell = %q, %v", cell, ok)
	}
