#!/usr/bin/env bash
#
# End-to-end integration tests for rpie inside official PHP Docker images.
#
# These are slow (they compile rpie for Linux and run real installs), so they
...
build_linux_binary() {
    ...
    note "Building rpie for $PLATFORM ($target_image)"
    docker run --rm --platform "$PLATFORM" -v "$PWD":/src -w /src "$target_image" \
        sh -c "CGO_ENABLED=0 go build -ldflags=\"-s -w\" -o /src/$out ."
}

# run_rpie <binary> <args...> — runs rpie in a fresh PHP container, echoes output
run_rpie() {
    local bin="$1"; shift
    ...
    docker run --rm --platform "$PLATFORM" -v "$PWD/$bin":/usr/local/bin/rpie "$PHP_IMAGE" \
        sh -c "$*" 2>&1 || true
}
...
build_linux_binary golang:1.26-bookworm rpie-glibc
BIN=rpie-glibc

note "info reports the target PHP"
out="$(run_rpie "$BIN" "rpie info")"
...
out="$(run_rpie "$BIN" "rpie install $EXAMPLE && php -m | grep -i example_pie")"
...
out="$(run_rpie "$BIN" "rpie install $EXAMPLE >/dev/null 2>&1; rpie show")"
...
out="$(run_rpie "$BIN" "rpie install $EXAMPLE >/dev/null 2>&1; rpie uninstall $EXAMPLE; rpie show")"
...
out="$(run_rpie "$BIN" "rpie install gd --install-system-deps && php -m | grep -i '^gd$'")"
...
out="$(run_rpie "$BIN" "rpie install php-amqp/php-amqp imagick/imagick --install-system-deps 2>&1 | grep -c 'Installed system dependencies' || true")"
assert_contains "single system-deps pass" "$out" "1"

note "parallel source builds (--jobs 2): both build, output grouped, both load"
out="$(run_rpie "$BIN" "rpie install php-amqp/php-amqp phpredis/phpredis --install-system-deps --jobs 2 && php -m | grep -iE '^amqp\$|^redis\$'")"
...
out="$(docker run --rm --platform "$PLATFORM" -v "$PWD/$BIN":/usr/local/bin/rpie php:8.1-cli \
    sh -c "rpie install swoole/swoole" 2>&1 || true)"
...
if [ "${ALPINE:-0}" = "1" ]; then
    build_linux_binary golang:1.26-alpine rpie-musl
    PHP_IMAGE="php:8.4-alpine"
    note "Alpine: bundled intl loads"
    out="$(run_rpie rpie-musl "rpie install intl --install-system-deps && php -m | grep -i '^intl$'")"
    assert_contains "intl loads on alpine" "$out" "intl"
fi

# ---- summary ------------------------------------------------------------------
note "Results: $pass passed, $fail failed"
rm -f rpie-glibc rpie-musl 2>/dev/null || true
[ "$fail" -eq 0 ]
