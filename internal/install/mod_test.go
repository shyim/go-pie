...
func TestWriteFilePlain(t *testing.T) {
	...
	if err := writeFile(t.Context(), path, "hello\n"); err != nil {
	...
	if err := writeFile(t.Context(), path, "bye\n"); err != nil {
		t.Fatal(err)
	}
...
func TestUninstallWindowsRemovesDll(t *testing.T) {
	dir := t.TempDir()
	php := platform.PhpBinaryFixture(8, 4, 0, platform.ArchX86_64)
	php.ExtensionDir = dir
	plat := platform.TargetPlatformFixture(platform.OSWindows, platform.NonThreadSafe, php)

	dll := php.DllPath("redis")
	if err := os.WriteFile(dll, []byte("dll"), 0o644); err != nil {
		t.Fatal(err)
	}
	ini := filepath.Join(dir, "20-redis.ini")
	...
	outcome, err := Uninstall(t.Context(), &ManagedExtension{
		PackageName:   "phpredis/phpredis",
		ExtensionName: "redis",
		Priority:      20,
		IniFile:       ini,
	}, plat)
	...
	if outcome.RemovedSo != dll {
		t.Fatalf("removed binary = %q, want %q", outcome.RemovedSo, dll)
	}
	if _, err := os.Stat(dll); !os.IsNotExist(err) {
		t.Fatalf("DLL still exists or stat failed unexpectedly: %v", err)
	}
}

func TestUninstallRejectsUnsafeManagedName(t *testing.T) {
	php := platform.PhpBinaryFixture(8, 4, 0, platform.ArchX86_64)
	plat := platform.TargetPlatformFixture(platform.OSLinux, platform.NonThreadSafe, php)
	_, err := Uninstall(t.Context(), &ManagedExtension{ExtensionName: "../outside"}, plat)
	if err == nil {
		t.Fatal("unsafe managed extension name should be rejected")
	}
}
...
func TestWalkFilesBoundedDepth(t *testing.T) {
	...
	// depth 1
	require.NoError(t, os.MkdirAll(filepath.Join(dir, "d1"), 0755))
	require.NoError(t, os.WriteFile(filepath.Join(dir, "d1", "b.txt"), []byte("b"), 0644))
	// depth 2
	require.NoError(t, os.MkdirAll(filepath.Join(dir, "d1", "d2"), 0755))
	require.NoError(t, os.WriteFile(filepath.Join(dir, "d1", "d2", "c.txt"), []byte("c"), 0644))
	// depth 3 (beyond bound: not collected)
	require.NoError(t, os.MkdirAll(filepath.Join(dir, "d1", "d2", "d3"), 0755))
	require.NoError(t, os.WriteFile(filepath.Join(dir, "d1", "d2", "d3", "d.txt"), []byte("d"), 0644))
