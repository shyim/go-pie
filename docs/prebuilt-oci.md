...
Make `rpie install <ext>` inside a Docker image *skip compilation* when we have
...
```
ghcr.io/<org>/rpie-ext/<extension>:<version>
# e.g. ghcr.io/shyim/rpie-ext/redis:6.3.0
```
  ...
  "manifests": [
    {
      ...
      "annotations": {
        "sh.go-pie.php": "8.4",
        "sh.go-pie.distro": "debian@12",
        "sh.go-pie.ts": "nts",
        "sh.go-pie.debug": "0",
        "sh.go-pie.config-hash": "0000000000000000",
        "sh.go-pie.cell": "redis/6.3.0/php8.4/debian@12/x86_64/nts/nodebug/cfg-00000000"
      }
    }
...
- **config blob** (`application/vnd.rpie.ext.config.v1+json`) — the custom
  manifest (schema below): runtime deps, checksums, provenance.
- **one layer** (`application/vnd.rpie.ext.layer.v1.tar+gzip`) — a tarball
...
```jsonc
{
  ...
  "builder": "rpie-nightly"             // provenance; attestation covers the layer
}
```
...
2. `GET ghcr.io/<org>/rpie-ext/<ext>:<version>` (the index). Anonymous pull
...
4. `GET` that manifest → pull its config blob (the manifest above) and verify:
   ...
   - checksums,
   - (optional) GitHub attestation on the layer digest, reusing the existing
     `--verify attest` path.
...
- The layer digest is covered by a GitHub build attestation from the nightly
  workflow; `--verify attest` (already implemented, native, no `gh`) verifies it.
- `--verify enforce` refuses a prebuilt artifact lacking a checksum/attestation.
...
`rpie build <ext> --emit-oci`, which produces the layer + config blob; a publish
...
- Client: compute the cell key; `oci` module that resolves the index, matches
  ...
  `RPIE_OCI_REGISTRY`.
- Build/emit: `rpie build --emit-oci <dir>` writes the layer tarball + config
