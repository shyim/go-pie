...
func Build(ctx context.Context, pkg *resolver.ResolvedPackage, src *download.DownloadedPackage,
	plat *platform.TargetPlatform, configureOptions []string) (BuiltExtension, error) {
	return BuildWith(ctx, pkg, src, plat, configureOptions, plat.MakeParallelJobs, nil)
}
...
func BuildWith(
	...
	src *download.DownloadedPackage,
	plat *platform.TargetPlatform,
	configureOptions []string,
	makeJobs int,
	sink *bytes.Buffer) (BuiltExtension, error) {
	...
	step := func(program string, args []string, label string) error {
		if sink != nil {
			return procutil.RunCaptured(ctx, program, args, cwd, label, sink)
		}
		return procutil.Run(ctx, program, args, cwd, label)
	}
