...
func (f fakeSource) PackageVersions(pkg string) ([]PackageVersion, error) {
	return ParseVersionsJSON(f.body, pkg)
}

func sourceOf(versionsJSON string) fakeSource {
	return fakeSource{body: []byte(fmt.Sprintf(`{"packages":{"vendor/ext":%s}}`, versionsJSON))}
}
...
func TestResolvesHighestStablePhpExt(t *testing.T) {
	...
	r, err := Resolve(src, req, fixturePlatform(platform.NonThreadSafe))
	if err != nil {
		t.Fatal(err)
	}
	if r.Version != "2.0.9" {
...
func TestRejectsNonExtensionType(t *testing.T) {
	...
	req := mustParseRequest(t, "vendor/ext")
	_, err := Resolve(src, req, fixturePlatform(platform.NonThreadSafe))
...
func TestRejectsIncompatibleThreadSafety(t *testing.T) {
	...
	req := mustParseRequest(t, "vendor/ext")
	_, err := Resolve(src, req, fixturePlatform(platform.ThreadSafe))
