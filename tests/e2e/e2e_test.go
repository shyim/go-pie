...
func TestMain(m *testing.M) {
	// Build the local rpie binary for Linux matching host architecture
	tmpDir, err := os.MkdirTemp("", "rpie-e2e-")
	if err != nil {
		panic(err)
	}
...
func TestE2E_Info(t *testing.T) {
	...
	err = ctr.CopyFileToContainer(ctx, binPath, "/usr/local/bin/go-pie", 0755)
	require.NoError(t, err)

	res, err := execInContainer(ctx, ctr, []string{"go-pie", "info"})
	require.NoError(t, err)
...
func TestE2E_InstallAndUninstall(t *testing.T) {
	...
	res, err := execInContainer(ctx, ctr, []string{"go-pie", "install", "asgrim/example-pie-extension"})
	require.NoError(t, err)
	assert.Equal(t, 0, res.ExitCode, "go-pie install failed. Output: %s", res.Stdout)
	...
	// Verify listed in go-pie show
	res, err = execInContainer(ctx, ctr, []string{"go-pie", "show"})
	require.NoError(t, err)
	assert.Equal(t, 0, res.ExitCode)
	...
	// Uninstall
	res, err = execInContainer(ctx, ctr, []string{"go-pie", "uninstall", "asgrim/example-pie-extension"})
	require.NoError(t, err)
	assert.Equal(t, 0, res.ExitCode)
	assert.Contains(t, res.Stdout, "uninstalled")

	// Verify not listed in go-pie show anymore
	res, err = execInContainer(ctx, ctr, []string{"go-pie", "show"})
	require.NoError(t, err)
	assert.Equal(t, 0, res.ExitCode)
