...
func TestFormatExitStatusOther(t *testing.T) {
	...
	lookOr(t, "sh")
	cmd := exec.CommandContext(context.Background(), "sh", "-c", "exit 100")
	_ = cmd.Run()
...
func TestRunSuccess(t *testing.T) {
	lookOr(t, "true")
	if err := Run(t.Context(), "true", nil, "", "noop"); err != nil {
...
func TestRunNonZeroExit(t *testing.T) {
	...
	lookOr(t, "sh")
	err := Run(t.Context(), "sh", []string{"-c", "exit 1"}, "", "mylabel")
	if err == nil {
...
func TestRunSpawnFailure(t *testing.T) {
	err := Run(t.Context(), "this-binary-does-not-exist-gpie", nil, "", "spawnstep")
	if err == nil {
		t.Fatal("expected spawn error")
	}
	if !strings.HasPrefix(err.Error(), "failed to spawn `this-binary-does-not-exist-gpie` for spawnstep: ") {
...
func TestRunCapturedCapturesOutput(t *testing.T) {
	...
	err := Run(t.Context(), "true", nil, "", "x")
	if err != nil {
		t.Fatal(err)
	}
	err = RunCaptured(t.Context(), "sh", []string{"-c", "echo out; echo err 1>&2"}, "", "step", &sink)
	if err != nil {
...
func TestRunCapturedNonZeroAppendsAndReturns(t *testing.T) {
	...
	lookOr(t, "sh")
	var sink bytes.Buffer
	err := RunCaptured(t.Context(), "sh", []string{"-c", "echo hi; exit 2"}, "", "make", &sink)
	if err == nil {
		t.Fatal("expected error")
	}
...
func TestCaptureSuccess(t *testing.T) {
	...
	lookOr(t, "sh")
	out, err := Capture(t.Context(), "sh", []string{"-c", "echo '  trimmed  '"})
	if err != nil {
		t.Fatal(err)
	}
	if out != "trimmed" {
...
func TestCaptureNonZeroExit(t *testing.T) {
	...
	lookOr(t, "sh")
	_, err := Capture(t.Context(), "sh", []string{"-c", "echo boom 1>&2; exit 3"})
	if err == nil {
		t.Fatal("expected error")
	}
...
func TestCaptureSpawnFailure(t *testing.T) {
	_, err := Capture(t.Context(), "this-binary-does-not-exist-gpie", nil)
	if err == nil {
		t.Fatal("expected error")
	}
	if !strings.HasPrefix(err.Error(), "failed to spawn `this-binary-does-not-exist-gpie`: ") {
...
func TestRunRelativeProgramResolvedAgainstCwd(t *testing.T) {
	...
	if err := Run(t.Context(), "./configure", nil, dir, "./configure"); err != nil {
...
func TestRunRelativeProgramErrorKeepsVerbatim(t *testing.T) {
	...
	err := Run(t.Context(), "./configure", []string{"--x"}, dir, "./configure")
	if err == nil {
		t.Fatal("expected error")
	}
...
// Cancelling the context must kill the child process rather than let it run to
// completion; this is what makes Ctrl-C tear down a long build.
func TestRunHonoursContextCancellation(t *testing.T) {
	...
	start := time.Now()
	err := Run(ctx, "sleep", []string{"30"}, ".", "cancellation probe")
	elapsed := time.Since(start)

	if err == nil {
		t.Fatal("expected the cancelled child to report failure")
	}
	if elapsed > 5*time.Second {
		t.Fatalf("child outlived its context: took %v", elapsed)
	}
}
